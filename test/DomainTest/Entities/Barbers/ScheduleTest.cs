...
public class ScheduleTest
{
    ...
    public void ShouldNoBeAbleToCreateAScheduleWithEndTimeGreaterThenEndTime()
    {
        ...
        var exception = Assert.Throws<ScheduleException>(() => new Schedule(now, now.AddHours(-1), DayOfWeek.Monday));
    ...
    {
        var now = DateTime.UtcNow;
        var exception = Assert.Throws<ScheduleException>(() => new Schedule(now, now, DayOfWeek.Monday));
        Assert.True(exception.Message.Equals(ScheduleExceptionMessagesResource.START_TIME_MUST_BE_GREATER_THEN_END_TIME));
    }
    ...
    {
        var exception = Assert.Throws<ScheduleException>(() => new Schedule(DateTime.MinValue, DateTime.MinValue, DayOfWeek.Monday));
